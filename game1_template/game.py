...
def normalise_input(user_input):
    ...
    'help'
    """ 
    s_normalise = remove_spaces(remove_punct(user_input)).lower()
    return s_normalise
...
def exit_leads_to(exits, direction):
    ...
    'Reception'
    """
    leadingroom = exits[direction]
    return rooms[leadingroom]["name"]
...
def print_menu(exits):
    ...
    for key in exits:
        directionleadsto = exit_leads_to(exits, key)
        print_menu_line(key, directionleadsto)
    # COMPLETE THIS PART:
...
def menu(exits):
    ...
    while True:
        ...
        # Normalise the input
        NUserInput = normalise_input(UserInput)

        # Check if the input makes sense (is vatlid exit)
        if is_valid_exit(exits, NUserInput) == True:
